...
def multiple_neurons(n_neurons, sim_time):
    ...
    total_fig.savefig(f"current_injection{n_neurons}neurons_{sim_time}ms_sim_time.png")
    # total_fig.show()
...
def multiple_neurons_with_poisson_neurons(n_neurons, sim_time):
    ...
    total_fig = plt.figure()
    ax_total = total_fig.add_subplot(111, title="Mean and Std for all currents")
    # excitatory_spike = 'w : 0.1'
    # inhibitory_spike = 'w : -0.1'
    w = 0.1
    for rate_magnitude in tqdm(rates_magnitude):
        ...
        E = bs.PoissonGroup(1000, rates=rate_magnitude*bs.Hz)
        I = bs.PoissonGroup(500, rates=rate_magnitude*bs.Hz)
        ...
        ax_total.errorbar(neurons, means, variances, linestyle="None", marker='^', label=f"{rate_magnitude}")

    ax_total.legend(loc="best")
    total_fig.savefig(f"poisson_neuron_{n_neurons}neurons_{sim_time}ms_sim_time.png")
    # total_fig.show()
...
if __name__ == '__main__':
    ...
    #
    # multiple_neurons_with_poisson_neurons(n_neurons=5, sim_time=200)
    # multiple_neurons_with_poisson_neurons(n_neurons=10, sim_time=200)
    # multiple_neurons_with_poisson_neurons(n_neurons=100, sim_time=200)
    # multiple_neurons_with_poisson_neurons(n_neurons=200, sim_time=200)
