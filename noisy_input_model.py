...
def noisy_input_test_vary_exc_inh(weight, sim_time_in_ms=100):
    v_r = V_REST
    ...
    for rate_magnitude in tqdm(rates_magnitudes):
        ...
        for v_trajectory in v_trajectories:
            traj = plt.plot(state_monitor.t / bs.ms, v_trajectory, 'b', alpha=0.3)

        # plt.plot(state_monitor.t / bs.ms, mean_trajectory, 'r', linestyle=":")
        error = plt.errorbar(state_monitor.t / bs.ms, mean_trajectory, trajectory_stdev, ecolor="orangered",
                             linestyle=":", color="red")
        bs.start_scope()
        G = bs.NeuronGroup(N=1, model=lif_stein_model, threshold=threshold, reset=reset,
                           refractory=refractory,
        ...
        plt.xlabel('Time (ms)')
        plt.ylabel('u')
        plt.title(f"Rate: {rate_magnitude}, weight: {weight}, Bias Current: {0.0}")
        plt.legend([error], ["Mean and Std"])

        dir_name = f"vary_exc_inh_ratio"
        if not os.path.isdir(dir_name):
            os.mkdir(dir_name)
        plt.savefig(f"{dir_name}/Rate_{rate_magnitude}_weight_{weight}_Bias_Current_{0.0}.png")
        plt.show()
...
def noisy_input_const_exc_inh(sim_time_in_ms=100):
    ...
    # rates_magnitudes = [10, 50, 100]
    rates_magnitudes = [10]
    bias_currents = [0.0, 1.0, 1.5, 2.0]
    for bias_current in bias_currents:
        for rate_magnitude in tqdm(rates_magnitudes):
            w = 0.1
            rate_magnitude_new = rate_magnitude
            ...
            for trial in range(n_trials):
                ...
                E = bs.PoissonGroup(100, rates=rate_magnitude_new * bs.Hz)
                I = bs.PoissonGroup(100, rates=rate_magnitude_new * bs.Hz)
            ...
            dir_name = f"with_a_const_exc_inh_weight"
            if not os.path.isdir(dir_name):
                os.mkdir(dir_name)
...
if __name__ == '__main__':
    ...
    noisy_input_test_vary_exc_inh(weight=2, sim_time_in_ms=50)
    noisy_input_test_vary_exc_inh(weight=4, sim_time_in_ms=50)
